...
#include <eoslib/string.hpp> /// defines transfer struct (abi)
...
namespace UTOKEN {
   ...
   */
   ...
   /**
   * Defines a universal token
   */
   typedef eosio::token<uint64_t,N(utoken)> currency_tokens;
 ...
 //  static_assert( sizeof(account) == sizeof(uint64_t)+sizeof(currency_tokens), "unexpected packing" );
   ...
   **/
   using accounts = eosio::table<N(utoken),N(utoken),N(account),account,uint64_t>;
